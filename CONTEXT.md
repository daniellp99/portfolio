...
- **Month navigation** uses a native `<form>` with `useActionState` + `useOptimistic`: `startTransition` calls `setOptimisticState` then `formAction(formData)` so React keeps the action transition open until the server action resolves and replaces optimistic state with the returned value. Nav tabs use the same pattern with `switchMainGridTabFormAction`. Caption chrome lives in `ContributionsMonthCaptionNav` (ported from the former react-day-picker label layout). `updateTag` runs only when navigating **to** the current month. Retries re-invoke `changeContributionsMonth` with the current year/month. While the form action is pending, `data-pending` on the form drives card pulse via `group/card`. There is no client Route Handler or client-side fetch cache for contributions.
